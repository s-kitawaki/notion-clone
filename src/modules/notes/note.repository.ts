...
export const noteRepository = {
    ...
    async find(userId: string, parentDocumentId?: number) {
        ...
        // parentDocumentIdを検索条件に追加するかの条件
        const { data } = parentDocumentId != null ? await query.eq('parent_document', parentDocumentId) : await query.is('parent_document', null);
        return data;
    },

    async findByKeyword(userId: string, keyword: string) {
        const { data } = await supabase.from('notes').select().eq('user_id', userId).or(`title.ilike.%${keyword}%,content.ilike.%${keyword}%`).order('created_at', { ascending: false });
        return data;
    },
